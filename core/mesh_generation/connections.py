...
from utils.gmsh_helpers import close_prism
...
def cutMod():
    ...
    for wall in walls:
        for beam in beams:
            try:
                _, fragment_map = gmsh.model.occ.fragment([(3, wall)], [(3, beam)], removeObject=False, removeTool=False)
                gmsh.model.occ.synchronize()

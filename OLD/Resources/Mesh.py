...
def createGmshModel(stepfile, labels):
    ...
    gmshmodel = createOriginalMatPhysicalGroups(gmsh.model, labels)
    #gmshmodel = fixBoundaries(gmsh.model)
    Connections.split_beam_and_assign_to_wall(gmshmodel, labels)
    #createMatPhisicalGroups(gmshmodel, labels)
    gmsh.model.geo.removeAllDuplicates()
    gmsh.model.removePhysicalGroups()
    gmsh.model.occ.synchronize()
    gmshmodel = createMatPhysicalGroups(gmshmodel, labels)
    gmsh.model.occ.synchronize()
    #mesh_size(gmshmodel)
    gmsh.fltk.run()
...
# def createMatPhysicalGroups(gmshmodel, labels):
#     gmshmodel.occ.synchronize()  # Ensure entities are available
...
#         print(f"Entity {tg} Name: {EntName}")  # Debugging print
...
#         print(f"Key: {key2}, Matches: {matches}")  # Debugging print
#         if matches:
...
#         value = FinalDict[key]
#         if not value:
#             print(f"Warning: No entities found for {key}")  # Debugging
#         gmshmodel.addPhysicalGroup(dim=3, tags=value)
#         #gmsh.model.setPhysicalName(dim=3, tag=group_id, name=key)

#     gmsh.model.occ.synchronize()  # Final sync before returning
#     return gmsh.model
...
def createOriginalMatPhysicalGroups(gmshmodel, labels):
    tridEleTags = gmshmodel.occ.getEntities(dim=3)

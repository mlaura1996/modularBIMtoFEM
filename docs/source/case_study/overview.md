...
above). This is the geometry the interactive interface-selection tool and
the full-aggregate runs below use - the 316-solid/663.71 m³ figures above
still describe the original, still used by the smaller
`test_task_ab_*.py` test scripts.
...
- an 18-volume connected cluster (built by BFS over touching pairs from
  ...
  SERA-AIMS reference partition count (`test_task_ab_scaled.py`)
- **the full aggregate** - all 279 volumes of the cleaned geometry (no
  ...
  {doc}`../developer_guide/interface_detection`), **6 partitions**
...
{doc}`../developer_guide/interface_detection` for the full account,
...
Two things this case study exists to show: (a) mesh generation from
as-built geometry, and (b) the computational demand an analysis at this
scale entails. (a) is demonstrated by the geometry pipeline and the
278,858-node trial mesh above. (b) — actual wall-clock time, peak memory,
DOF count — is not yet measured, because the

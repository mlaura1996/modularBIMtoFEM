...
- `output/castelnuovo/hmo_graph.ttl` - RDF/Turtle graph instantiating HSV
  (survey-document provenance) and HMO (masonry characterisation)
  individuals for the four types. Hand-written Turtle (no `rdflib` in the
  Docker image) rather than a dependency for something this
  straightforward. HSTO (structural decomposition - facades, floors,
  unit-to-unit connections) is deliberately left minimal - only
  `hsto:isMadeOf` is used, the one HSTO property independently confirmed
  from the manuscript text, rather than guessing at further HSTO class/
  property names that weren't read from an ontology source the way HMO's
  were.
...
- The full HSTO structural-decomposition graph (facades, floor systems,
  wall-to-wall connections as ontology individuals, not just the flat
  `hsto:isMadeOf` used here) hasn't been attempted - would need the same
  read-the-ontology-source treatment HMO got here, applied to
  `github.com/mlaura1996/HistoricStructureOntology` (or wherever HSTO's
  source lives - not yet located).

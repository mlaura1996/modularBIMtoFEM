...
with apeGmsh(model_name="plot_eigen_fine") as g:
    ...
    # Grouped by owning volume (not one flat set) - matplotlib's Axes3D
    # depth-sorts each Line3DCollection/Poly3DCollection as a whole (one
    # average depth per collection, not per segment), so a single giant
    # edge collection spanning the entire aggregate got sorted as either
    # entirely in front of or entirely behind the face collection - the
    # cause of the "x-ray" look surviving even after restricting to
    # exterior-only curves. Splitting into one collection per volume gives
    # the sort many small, locally-correct buckets instead of one global
    # one, which interleaves far better with the (also per-triangle
    # depth-sorted) face collection.
    edge_pairs_by_vol = {}
    ...
    print(f"Geometry wireframe: {n_edges_total} CAD edge segments across "
          f"{len(geom_edge_row_groups)} per-volume groups - for comparison, "
...
def add_geometry_edges(ax, deformed_coords, linewidth=GEOM_EDGE_LINEWIDTH):
    """One Line3DCollection per volume (see geom_edge_row_groups) rather
    than one big collection for the whole aggregate - see the comment
    where geom_edge_row_groups is built for why."""
...
save_views(build_selfweight_ax, "full_aggregate_fine_selfweight_FINE",
...
for mode in range(1, N_MODES + 1):
    ...
    save_views(
        build_mode_ax, f"full_aggregate_fine_mode{mode}_FINE",

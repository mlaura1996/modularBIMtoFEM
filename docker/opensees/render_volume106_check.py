...
with apeGmsh(model_name="render_volume106_check") as g:
...
for name, zoom in [("context", False), ("zoom", True)]:
    ...
    path = f"output/castelnuovo/plots/volume106_check_{name}.png"
...
for other_vol, center in joint_centroids.items():
    for elev, azim, view_name in [(20, -45, "a"), (60, 10, "b")]:
        ...
        path = f"output/castelnuovo/plots/volume106_joint_{other_vol}_{view_name}.png"
        ax.figure.savefig(path, dpi=200, bbox_inches="tight")
        plt.close(ax.figure)
        print(f"Wrote {path}")
